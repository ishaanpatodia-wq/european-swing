(function(){
  ...
  const hasLiveData=()=>S.liveLeaderboard?.playStarted===true&&Array.isArray(S.liveLeaderboard?.players)&&S.liveLeaderboard.players.length>0;
  ...
  async function loadLive(force=false){if(!revealedNow(ev())||ev()?.complete)return;if(!force&&S.liveLeaderboard.loading)return;if(S.liveLeaderboard.eventId!==S.event)S.liveLeaderboard={eventId:S.event,players:[],updatedAt:null,loading:true,error:null};else{S.liveLeaderboard.loading=true;S.liveLeaderboard.error=null}paintLive();try{const r=await fetch(`${cfg.supabaseUrl}/functions/v1/${LIVE_FN}?event=${encodeURIComponent(S.event)}&_=${Date.now()}`,{cache:'no-store',headers:{apikey:cfg.supabaseAnonKey,'cache-control':'no-cache','pragma':'no-cache'}});if(!r.ok)throw new Error(`Live feed ${r.status}`);const data=await r.json();const incoming=Array.isArray(data.players)?data.players:[];if(data.playStarted===true&&incoming.length){S.liveLeaderboard={eventId:S.event,players:data.playStarted===true?incoming:[],updatedAt:data.updatedAt||new Date().toISOString(),round:data.round||null,eventName:data.eventName||null,playStarted:data.playStarted===true,projectedCutScore:data.projectedCutScore,projectedCut:data.projectedCut||null,projectedCutMethod:data.projectedCutMethod||null,cutFinal:data.cutFinal===true,eventComplete:data.eventComplete===true,loading:false,error:data.error||null};try{localStorage.setItem('euro_live_'+S.event,JSON.stringify(S.liveLeaderboard))}catch(_e){}}else{S.liveLeaderboard={eventId:S.event,players:[],updatedAt:data.updatedAt||new Date().toISOString(),round:data.round||null,eventName:data.eventName||null,playStarted:false,projectedCutScore:null,projectedCut:null,projectedCutMethod:null,cutFinal:false,eventComplete:false,loading:false,error:data.error||null};try{localStorage.removeItem('euro_live_'+S.event)}catch(_e){}}}catch(e){S.liveLeaderboard.loading=false;S.liveLeaderboard.error=String(e?.message||e||'unavailable')}paintLive()}
  ...
  try{const cached=JSON.parse(localStorage.getItem('euro_live_'+S.event)||'null');if(cached&&cached.playStarted===true&&Array.isArray(cached.players)&&cached.players.length)S.liveLeaderboard=cached}catch(_e){}render();loadLive(true)
})();
