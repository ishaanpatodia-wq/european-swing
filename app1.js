...
const draftKey=()=>{const who=(S.user||AUTH?.name||'').toLowerCase();return who?`euro_draft:${who}:${S.event}`:null};
function saveDraft(){const k=draftKey();if(!k||revealedNow(ev()))return;try{localStorage.setItem(k,JSON.stringify({picks:S.roster.map(p=>p.name),chip:S.chip,double:S.double,extra:S.extra?.name||null,editing:!!S.editing}))}catch{}}
function clearDraft(){const k=draftKey();if(!k)return;try{localStorage.removeItem(k)}catch{}}
function restoreDraft(){const k=draftKey();if(!k||!S.rank.length)return false;let d=null;try{d=JSON.parse(localStorage.getItem(k)||'null')}catch{}if(!d)return false;S.roster=[];for(const name of d.picks||[]){const p=S.rank.find(x=>x.name===name);if(p&&S.roster.length<5&&!S.roster.some(x=>x.name===p.name)&&rosterCost()+p.points<=12)S.roster.push(p)}S.chip=d.chip==='AND'||d.chip==='DOUBLE'?d.chip:null;S.double=null;S.extra=null;if(S.chip==='DOUBLE'&&d.double&&S.roster.some(p=>p.name===d.double))S.double=d.double;if(S.chip==='AND'&&d.extra){const p=S.rank.find(x=>x.name===d.extra);if(p&&!S.roster.some(x=>x.name===p.name)&&rosterCost()+p.points<=12)S.extra=p}S.editing=!!d.editing;return true}
...
async function refresh(){S.loading=true;S.error=null;S.revealed=null;render();try{await loadRank();if(!TOKEN){S.loading=false;render();return}S.status=await rpc('get_my_status',{p_token:TOKEN,p_event_id:S.event});S.user=S.status?.player||null;const restored=restoreDraft();if(S.status?.submitted&&!restored)S.editing=false;if(revealedNow(ev())){clearDraft();S.editing=false;try{S.revealed=await rpc('get_revealed_entries',{p_token:TOKEN,p_event_id:S.event})}catch(e){S.revealed=[]}}}catch(e){S.error=e.message||String(e)}S.loading=false;render()}
function toggle(name){const p=S.rank.find(x=>x.name===name);if(!p)return;const i=S.roster.findIndex(x=>x.name===name);if(i>=0){S.roster.splice(i,1);normalizeExtra();saveDraft();render();return}if(S.roster.length>=5)return;if(totalCost()+p.points>12)return alert('12-point budget exceeded.');S.roster.push(p);normalizeExtra();saveDraft();render()}
...
function chooseChip(c){if(!ev().chips)return alert('Chips are not available in Abu Dhabi or Dubai.');S.chip=S.chip===c?null:c;S.double=null;S.extra=null;saveDraft();render()}
async function submit(){if(S.roster.length!==5)return alert('Pick exactly 5 players.');if(totalCost()>12)return alert('Maximum 12 points.');if(S.chip==='DOUBLE'&&!S.double)return alert('Choose the player to double.');if(S.chip==='AND'&&!S.extra)return alert('Choose the 6th player.');if(revealedNow(ev()))return alert('Teams have already been revealed.');try{const res=await rpc('submit_picks',{p_token:TOKEN,p_event_id:S.event,p_picks:S.roster.map(p=>p.name),p_chip:S.chip,p_extra_player:S.extra?.name||null,p_double_player:S.double||null});clearDraft();S.roster=[];S.chip=null;S.double=null;S.extra=null;S.editing=false;await refresh();if(res?.late)alert('Submitted — a little after the Wednesday target, but accepted.')}catch(e){alert(e.message)}}
function editEntry(){if(!S.status?.submitted||revealedNow(ev()))return;S.editing=true;S.roster=(S.status.picks||[]).map(n=>S.rank.find(p=>p.name===n)).filter(Boolean);S.chip=S.status.chip||null;S.double=S.status.double_player||null;S.extra=S.status.extra_player?S.rank.find(p=>p.name===S.status.extra_player)||null:null;saveDraft();render()}
...
function logout(){localStorage.removeItem('euro_auth');AUTH=null;TOKEN='';S.user=null;S.status=null;S.revealed=null;S.roster=[];S.chip=null;S.double=null;S.extra=null;S.editing=false;S.loginName=null;S.loginSetup=false;S.accountOpen=false;S.accountError=null;S.error=null;render()}
