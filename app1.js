...
async function refresh(){S.loading=true;S.error=null;S.revealed=null;render();try{await loadRank();if(!TOKEN){S.loading=false;render();return}S.status=await rpc('get_my_status',{p_token:TOKEN,p_event_id:S.event});S.user=S.status?.player||null;if(S.status?.submitted)clearDraft();else restoreDraft();if(revealedNow(ev())){clearDraft();try{S.revealed=await rpc('get_revealed_entries',{p_token:TOKEN,p_event_id:S.event})}catch(e){S.revealed=[]}}}catch(e){S.error=e.message||String(e)}S.loading=false;render()}
